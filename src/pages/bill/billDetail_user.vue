<template>
    <div class="billDetail">
        <Card>
            ...
            <Form :label-width="100" :model="billDetail" :rules="ruleValidate" ref="Form">
                <Row type="flex" justify="space-around">
                    <Col span="12">
                        ...
                        <Card v-if="billDetail.state==='2'||billDetail.state==='3'||billDetail.state==='4'||billDetail.state==='5'">
                            ...
                            <Row>
                                <Col span="12">
                                    <!-- v-if v-else为了显示必填标识符-->
                                    <Form-item v-if="billDetail.state==='2'||(billDetail.state==='4'&&!state4Readonly)"
                                               label="上传发票："
                                               prop="invoiceFileAddress">
                                        ...
                                        <upload v-show="billDetail.state!=='3'&&billDetail.state!=='5'&&!state4Readonly"
                                                ref="upload"
                                                :format="['jpg','jpeg','png']"
                                                ...
                                                type="select"
                                                :action="ossUrl">
                                            <Button size="small">选择发票</Button>
                                        </upload>
                                    </Form-item>
                                    <Form-item label="上传发票：" v-else>
                                        <div v-show="billDetail.invoiceFileAddress">
                                            <a href="javascript:">
                                                {{billDetail.invoiceFileAddress.split('/')[billDetail.invoiceFileAddress.split('/').length-1]}}
                                            </a>
                                        ...
                                        <upload
                                                v-show="billDetail.state!=='3'&&billDetail.state!=='5'&&!state4Readonly"
                                                ref="upload"
                                                :format="['jpg','jpeg','png']"
                                                :before-upload="handleUpload"
                                                :max-size="2048"
                                ...
                                <Col span="12">
                                    <Form-item label="邮寄地址：" v-if="billDetail.state==='2'" prop="invoiceSendAddress">
                                        <Input v-model.trim="billDetail.invoiceSendAddress" placeholder="邮寄地址："
                                               :maxlength="200">
                                        </Input>
                                    </Form-item>
                                    <Form-item label="邮寄地址：" v-else>
                            ...
                            <Row v-if="billDetail.state==='4'">
                                <Col span="24">
                                    <Form-item label="驳回原因：">
                                        <Input v-model="billDetail.denyRemark" type="textarea"
                                               disabled></Input>
                                    </Form-item>
                                </Col>
                            </Row>
                        </Card>
                        <Card v-if="billDetail.state==='5'">
                            <p slot="title">打款信息</p>
                            <Row>
                                <Col span="12">
                            ...
                            <Row>
                                <Col span="12">
                                    <Form-item label="上传打款记录：">
                                        ...
                                        <upload
                                                v-if="!billDetail.payFileAddress"
                                                ref="upload"
                                                :format="['jpg','jpeg','png']"
                                                :before-upload="handleUpload"
                                                :max-size="2048"
                                                type="select"
                                                :action="ossUrl">
                                            <Button type="primary" size="small">上传打款记录</Button>
                                        </upload>
                                    </Form-item>
                                </Col>
                                <Col span="12">
                                    <Form-item label="打款人：">
                                        <p>{{billDetail.payUserName}}</p>
                                    </Form-item>
                                </Col>
                            </Row>
                        </Card>
                        <Row v-if="billDetail.state==='2'" type="flex" justify="center"
                             class="formItemRow"
                             align="middle" style="margin-top: 20px">
                            <Col span="8" push="2">
                                <Button style="width: 100px" type="primary" @click="uploadInvoice">上传发票</Button>
                            </Col>
                            <Col span="8">
                                <Button style="width:100px" type="primary" @click="go2BillList">取消</Button>
                            </Col>
                        </Row>
                        <Row v-if="billDetail.state==='4'" type="flex" justify="center"
                             class="formItemRow"
                             align="middle" style="margin-top: 20px">
                            <Col span="8" push="2" v-show="!state4Readonly">
                                <Button style="width: 100px" type="primary" @click="uploadInvoice">重新上传发票</Button>
                            </Col>
                            <Col span="8">
                                <Button style="width:100px" type="primary" @click="go2BillList">取消</Button>
                            </Col>
                        </Row>
                        <Row v-if="billDetail.state==='1'" type="flex"
                             justify="center"
                             class="formItemRow"
                             align="middle" style="margin-top: 20px">
                            <Col pull="1">
                                <Button style="width:100px" type="primary" @click="acceptBill">接受结算单</Button>
                            </Col>
                            <Col>
                                <Button style="width:100px" type="primary" @click="go2BillList">取消</Button>
                            </Col>
                        </Row>
                        <Row v-if="billDetail.state==='3'||billDetail.state==='5'" type="flex"
                             justify="center"
                             class="formItemRow"
                             align="middle" style="margin-top: 20px">
                            <Col>
  ...
  export default {
    ...
    data () {
      return {
        ...
        ruleValidate: {
          invoiceSendAddress: [
            { trigger: 'blur', required: true, message: '请输入邮寄地址' }
          ],
          invoiceFileAddress: [
            { required: true }
          ],
    ...
    created () {
      this.state4Readonly = Boolean(this.$route.query.isReadonly)
    },
    mounted: function () {
      this.getBillDetail()
    ...
    methods: {
      uploadInvoice () {
        if (!this.billDetail.invoiceFileAddress) {
          this.$Message.error('请上传发票')
        } else {
          // 已驳回重新上传发票的结算单不用再次校验邮寄地址
          if (this.billDetail.state === '4') {
            this.uploadXhr()
          } else {
            this.$refs.Form.validateField('invoiceSendAddress', (valid) => {
              if (!valid) {
                this.uploadXhr()
              }
            })
      ...
      uploadXhr () {
        this.http.post('tcprom/settlement/uploadInvoice', {
          proSettlementId: this.$route.query.proSettlementId,
          invoiceFileAddress: this.billDetail.invoiceFileAddress,
          invoiceSendAddress: this.billDetail.invoiceSendAddress
        }).then((data) => {
      ...
      handleUpload (file) {
        if (file.type.indexOf('image') !== -1) {
          ...
          this.http.post('filedepot/ossObject/uploadToOSSWeb/prom', param).then((response) => {
            this.billDetail.invoiceFileAddress = response.fileVisitPath
      ...
      acceptBill () {
        this.http.post('tcprom/settlement/receive', {
          proSettlementId: this.$route.query.proSettlementId
        }).then((data) => {
          this.$Message.success({
            content: '结算单已接受',
