<template>
    <div>
        <Card>
            ...
            <span slot="extra">
                <Button style="margin-right:1em" type="primary" size="small" icon="ios-cloud-upload-outline"
                        @click="updateList">导入员工名单</Button>
            </span>
            ...
            <div v-show="accountList.length>0" class="layout-content-main">
                <div class="layout-content-main-bottom">
                        ...
                        <Col span="24">
                            <table class="addAcount" cellspacing="0" cellpadding="0" border="0">
                                ...
                                <tr v-for="(account,index) in accountList">
                                    <td>
                                        <Button v-show="account.isEditting == 'false' || !account.isEditting"
                                                type="text">{{account.importPhone}}
                                        </Button>
                                        <Input :maxlength="11" class="addInput" v-show="account.isEditting == 'true'"
                                               v-model="account.importPhone"></Input>
                                    </td>
                                    <td>
                                        <Button v-show="account.isEditting == 'false' || !account.isEditting"
                                                type="text">{{account.importName}}
                                        </Button>
                                        <Input class="addInput" v-show="account.isEditting == 'true'"
                                               v-model="account.importName"></Input>
                                    </td>
                                    <td :class="{error:account.reserved1==1}">
                                        {{account.importMsg}}
                                    </td>
                                    <td>
                                        <Button v-show="account.isEditting == 'false'" type="primary" size="small"
                                                @click="editThisAccount(account)">编辑
                                        </Button>
                                        <Button v-show="account.isEditting == 'true'" type="success" size="small"
                                                @click="checkAccount(account)">确定
                                        </Button>
                                        <Button type="error" size="small" class="delete-btn"
                                                @click="deleteThisAccount(account)">删除
                                        </Button>
                                    </td>
                                </tr>
                            </table>
                    ...
                    <template>
                        <Row type="flex" justify="space-between" style="margin:30px 0 20px;">
                            ...
                            <Col span="10">
                                <Page style="float:right;padding-right:10%" :total="total" size="small" show-elevator
                                      show-total :pageSize='pageSize' @on-change="toPage"
                    ...
                    <Row type="flex" justify="space-around" align="middle" class="formItemRow">
                        <Col span="7">
                            <Form-item label="请选择团队">
                                <Select filterable clearable v-model="formItem.team" @on-change="getDeptList"
                                        placeholder="请选择">
                                    <Option v-if="teamList.length==0" value="-1">暂无团队</Option>
                                    <Option v-else v-for="team in teamList" :key="team.qrmDtteamId"
                                            :value="team.qrmDtteamId">{{team.dtteamName}}
                                    </Option>
                                </Select>
                            </Form-item>
                        </Col>
                        <Col span="7">
                            <Form-item label="请选择部门">
                                <Select filterable clearable v-model="formItem.dept" placeholder="请选择">
                                    ...
                                    <Option v-else v-for="dept in deptList" :key="dept.qrmDtdeptId"
                                            :value="dept.qrmDtdeptId">{{dept.dtdeptName}}
                                    </Option>
                                </Select>
                            </Form-item>
                        </Col>
                ...
                <Row type="flex" justify="space-around" align="middle">
                    <Col span="24" style="text-align:center">
                        ...
                        <Button type="ghost" size="large" @click='go2AccountList'>取消</Button>
                    </Col>
                </Row>
            </div>
        </Card>
        <Modal class-name="vertical-center-modal" width="800" v-model="showUpdateModal" title="导入名单" @on-ok="updateOk"
               @on-cancel="updateCancel" ok-text="确认导入">
            <Card>
                <p slot="title">
                    <span style="color:#ed3f14;">注：员工信息均为必填，不能重复，不能包含空格、任意符号与图片。</span>
                </p>
                <span slot="extra">
                    <Button style="margin-right:1em" type="warning" size="small" icon="ios-cloud-download-outline"
                            @click="downloadExample">点击下载表格模板</Button>
                </span>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    mounted: function () {
      this.actionAddress = location.origin + '/tcprom/accountManage/listImport'
      // this.actionAddress = 'http://demo.tingjiandan.com:8080/prom/accountManage/listImport'
    },
    methods: {
      ...
      getDeptList () {
        this.http.post('tcprom/common/getDeptList', {
          qrmDtteamId: this.formItem.team < 0 ? '' : this.formItem.team,
          enableStatus: '0',
        }).then((data) => {
      ...
      deleteThisAccount (account) {
        this.$Modal.confirm({
          title: '删除新增账户',
          content: `<p style="margin-top:-5px;">确认删除 ${account.importName} 这条账户数据？</p>`,
          loading: true,
          onOk: () => {
