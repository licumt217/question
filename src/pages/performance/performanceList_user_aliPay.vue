...
<script type="text/ecmascript-6">
  ...
  export default {
    name: 'PerformanceListUserAlipay',
    data () {
      return {
        page: 1,
        pageSize: 10,
    ...
    methods: {
      ...
      go2detail (perform) {
        ...
        util.goto(this, 'performance/performance_user_aliPay/performanceDetail')
      },
      downloadSum () {
        if (!this.formItem.startDate || !this.formItem.endDate) {
          ...
          window.location = `${location.origin}/tcprom/dtCodeRecord/downloadExcel?qrmDtuserName=${this.formItem.qrmDtuserName}` +
            ...
            `&qrmDtdeptName=${this.formItem.qrmDtdeptName}&excelType=1&createType=2`
        }
      },
      downloadDetail () {
        if (!this.formItem.startDate || !this.formItem.endDate) {
          ...
          window.location = `${location.origin}/tcprom/dtCodeRecord/downloadExcel?qrmDtuserName=${this.formItem.qrmDtuserName}` +
            `&enableStatus=${this.formItem.enableStatus < 0 ? '' : this.formItem.enableStatus}` +
            `&startDate=${this.formItem.startDate}&endDate=${this.formItem.endDate}` +
            `&qrmDtdeptName=${this.formItem.qrmDtdeptName}&excelType=2&createType=2`
        }
      }
    }
  }
