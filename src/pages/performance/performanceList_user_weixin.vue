<template>
    <div class="performanceList">
        <Card>
            ...
            <span slot="extra">
                <Button style="margin-right:1em" type="primary" size="small" icon="ios-cloud-download-outline" @click="downloadSum">业绩统计下载</Button>
                ...
                <Row type="flex" justify="space-around" class="formItemRow" align="middle">
                    ...
                    <Col span="7">
                        <Form-item label="统计开始时间">
                            <Date-picker type="datetime" @on-change="handleStartTime" placeholder="选择日期和时间"></Date-picker>
                        </Form-item>
                    </Col>
                    <Col span="7">
                ...
                <Row type="flex" justify="space-around" class="formItemRow" align="middle">
                    ...
                    <Col span="7">
                        <Form-item label="部门名称">
                            <Input v-model="formItem.qrmDtdeptName" placeholder="请输入"></Input>
                        </Form-item>
                    </Col>
                    <!--<Col span="5">
                ...
                <div class="layout-content-main-bottom">
                    <Row type="flex" justify="center" class="content">
                        <Col span="24">
                            ...
                            <table class="addAcount" style="table-layout:auto;" v-show="qrmPerformList.length>0" cellspacing="0" cellpadding="0" border="0">
                                <tr>
                                    <td>序号</td>
                                    <td>姓名</td>
                                    <td>手机号</td>
                                ...
                                <tr v-for="(perform,index) in qrmPerformList" v-show="Object.keys(perform).length>0">
                                    ...
                                    <td>
                                        <Button type="primary" size="small" class="edit-btn" @click="go2detail(perform)">详情</Button>
                                    </td>
                                </tr>
                            </table>
                        </Col>
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        methods: {
            ...
            go2detail(perform) {
                util.setSession('user_thisPerformId',perform.qrmDtcodeUserId)
                util.goto(this,'performance/performance_user_weixin/performanceDetail');
            },
            downloadSum() {
                if(!this.formItem.startDate || !this.formItem.endDate){
                    ...
                    window.location = `${location.origin}/tcprom/dtCodeRecord/downloadExcel?qrmDtuserName=${this.formItem.qrmDtuserName}`+
                                    ...
                                    `&qrmDtdeptName=${this.formItem.qrmDtdeptName}&excelType=1&createType=1`;
                }
            },
            downloadDetail(){
                if(!this.formItem.startDate || !this.formItem.endDate){
                    ...
                    window.location = `${location.origin}/tcprom/dtCodeRecord/downloadExcel?qrmDtuserName=${this.formItem.qrmDtuserName}`+
                                    `&enableStatus=${this.formItem.enableStatus<0?'':this.formItem.enableStatus}`+
                                    `&startDate=${this.formItem.startDate}&endDate=${this.formItem.endDate}`+
                                    `&qrmDtdeptName=${this.formItem.qrmDtdeptName}&excelType=2&createType=1`;
                }
            }
        }
    }
